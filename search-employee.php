...
<head>
    ...
    <title>Birhan Solutions|Admin Page|Filter employee record by email</title>
    <style type="text/css">
        .wrapper {
            width: 500px;
            margin: 0 auto;
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header">
                        <h2>Filter Record By Email</h2>
                    </div>

                    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
                        <div class="form-group" style="padding-left: 20%;padding-right: 20%; width: 100%;">
                            <input type="email" class="form-control" name="email_hist" placeholder="Enter Employee's Email Address*" required style="height: 40px;border-radius: 10px;font-size: 15px;" />
                    ...
                    if (isset($_POST['submit'])) {
                        $email = $_POST["email_hist"];
                        $sql = "SELECT * FROM Developers_info where EmailAddress='$email'";
